        ...
        <div class="container">
            <div class="pc-topbar-inner">
                @php
                    ...
                    $topbarNote = \App\Models\Setting::get('topbar_note') ?: (app()->getLocale() == 'bn'
                ...
                <div class="pc-topbar-right">
                    ...
                    <a href="tel:{{ \App\Models\Setting::get('phone', '01716-952365') }}" class="pc-topbar-link">
                        <i class="bi bi-telephone-fill"></i>
                        <span>{{ \App\Models\Setting::get('phone', '01716-952365') }}</span>
                    </a>
                </div>
            </div>
        </div>
            ...
            <div class="pc-header-inner">
                ...
                <form action="{{ route('shop') }}" method="GET" class="pc-search d-none d-lg-block">
                    <div class="pc-search-box">
                        <select name="category" class="pc-search-cat" aria-label="{{ app()->getLocale() == 'bn' ? 'ক্যাটাগরি' : 'Category' }}">
                        ...
                        <button class="pc-search-btn" type="submit" aria-label="{{ app()->getLocale() == 'bn' ? 'খুঁজুন' : 'Search' }}">
                ...
                <div class="pc-header-actions">
                    @auth
                        <a href="{{ auth()->user()->isAdmin() ? route('admin.dashboard') : route('customer.dashboard') }}" class="pc-header-action">
    ...
    <nav class="pc-navbar d-none d-lg-block">
        <div class="container">
            <div class="pc-navbar-inner">
                ...
                <a href="tel:{{ \App\Models\Setting::get('phone', '01716-952365') }}" class="pc-nav-hotline">
    ...
    <div class="pc-payments">
        <div class="container">
            <div class="pc-payments-inner">
                <span class="pc-payments-label">{{ app()->getLocale() == 'bn' ? 'পেমেন্ট মাধ্যম:' : 'We accept:' }}</span>
                @foreach($payments as $payment)
                    <span class="pc-payment-chip"><i class="bi bi-{{ $payment->icon_name }}"></i> {{ $payment->title }}</span>
                @endforeach
            </div>
        </div>
    </div>
        ...
        <div class="container">
            <div class="row g-4">
                <div class="col-lg-4 col-md-6">
                    ...
                    <p class="footer-desc">
                        {{ \App\Models\Setting::get('footer_desc') ?: (app()->getLocale() == 'bn'
                           ? 'খাঁটি ও অর্গানিক পণ্যের অনলাইন বাজার। আম, খেজুর গুড়, ঘি, সরিষার তেল, মধু সহ সকল প্রাকৃতিক পণ্য সরাসরি চাঁপাই নবাবগঞ্জ থেকে সরবরাহ করা হয়।'
                ...
                <div class="col-lg-3 col-md-6">
                    ...
                    <ul class="footer-links">
                        @foreach($navCategories->take(6) as $navCategory)
                            <li><a href="{{ route('shop', ['category' => $navCategory->slug]) }}">{{ $navCategory->name }}</a></li>
                        @endforeach
                        <li><a href="{{ route('shop') }}">{{ app()->getLocale() == 'bn' ? 'সব পণ্য' : 'All Products' }}</a></li>
                    </ul>
                </div>
                @php $footerMenu = \App\Models\SiteBlock::list('footer_menu'); @endphp
                @if($footerMenu->count())
                <div class="col-lg-2 col-md-6">
                    <h5 class="footer-title">{{ app()->getLocale() == 'bn' ? 'আমার অ্যাকাউন্ট' : 'My Account' }}</h5>
                    <ul class="footer-links">
                        @foreach($footerMenu as $item)
                            <li>
                                <a href="{{ $item->link }}" @if($item->is_external) target="_blank" rel="noopener" @endif>
                                    {{ $item->title }}
                                </a>
                            </li>
                        @endforeach
